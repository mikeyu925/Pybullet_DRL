...
def setRobotJointsSpeed(robot_id,joints,velocitys,forces):
    for i in range(len(joints)):
        p.setJointMotorControl2(
            bodyUniqueId=robot_id,
            jointIndex=joints[i],
            controlMode=p.VELOCITY_CONTROL,
            targetVelocity=velocitys[i],
            force=forces[i]
        )

def showJointInformation(name,robot_id):
    jointNum = p.getNumJoints(robot_id)
    print(name + "关节数量为:" + str(jointNum))

    print("The info of " + name + ": ")
    for joint_index in range(jointNum):
